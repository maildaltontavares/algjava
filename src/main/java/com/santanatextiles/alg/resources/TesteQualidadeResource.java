...
public class TesteQualidadeResource {
 	...
 	*/
 	...
 	@CrossOrigin
 	@RequestMapping(value="busca/{filial}/{produtor}/{lote}/{item}", method=RequestMethod.GET)
 	public  ResponseEntity< ? > buscaTesteQualidadeByLoteItem (@PathVariable String filial,@PathVariable String produtor,@PathVariable String lote, @PathVariable String item){ 
 		try {
	 		 TesteQualidade  testeQualidade = service.buscaTesteQualidadeByLoteItem(filial, produtor, lote,item) ; 
	 		return ResponseEntity.status(HttpStatus.OK).body(testeQualidade);
		} catch(Exception e) {
			return ResponseEntity.status(HttpStatus.FORBIDDEN).body(e.getMessage());
		}  		
